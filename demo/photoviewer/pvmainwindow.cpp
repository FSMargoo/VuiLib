...
PVMainWindow::PVMainWindow(Core::VApplication *Parent, const std::wstring &FilePath) : VMLMainWindow(Parent)
{
	...
	ViewingImage =
		new Core::VImage(ViewingProject->FileList[ViewingProject->LocalViewingFile], CallWidgetGetRenderHandle());
...
void PVMainWindow::OpenViewProject()
{
	...
	if (SUCCEEDED(OperationResult))
	{
		...
		ViewingImage =
			new Core::VImage(ViewingProject->FileList[ViewingProject->LocalViewingFile], CallWidgetGetDCRenderTarget());
