...
int main() {
	...
	for (int ThreadCount = 0; ThreadCount < MAX_USER; ++ThreadCount) {
		std::thread SockThread([&](const int& ThreadID) -> void {
			while (true) {
				...
				if (Header->ChatVersion != LOCAL_SERVER_VERSION) {
					ThreadPrintInfo(ThreadID, "Error : Received Incompatible client version {0x%x}, send the error pack to client, the connection with client will shutdown!", (const char*)Header->ChatVersion);
				...
				while (true) {
					...
					if (ClientOperation->OperationCode == CLIENT_NEWMESSAGE) {
						ThreadPrintInfo(ThreadID, "User {%s} try to send message, now trying to received message from client...", (const char*)UserName);

						auto UserSentMessage = RecData<char>(ClientSocket, ClientOperation->MessageLength, 0);

						if (!std::get<1>(UserSentMessage)) {
							ThreadPrintInfo(ThreadID, "Error : User {%s} tried to send message, but server received a unexpected web pack, the connection will shutdown!", (const char*)UserName);
						...
						printf("[Server thread %d] : <%s> : %s\nPack length : %d\n", ThreadID, UserName, UserMessage, ClientOperation->MessageLength);
