...
private:
	...
	void FirstContact() {
		...
		if (BackPack->ServerCheckCode == SUCCESS) {
			...
			this->operator[](L"main-widget")[L"login-interface"][L"login-block"][L"error-text"].Get<Core::VTextLabel>()->SetPlaneText(L"The version of client is not compatible with the server's");
		}
		else if (BackPack->ServerCheckCode == USER_ALREADY_EXISTS) {
			this->operator[](L"main-widget")[L"login-interface"][L"login-block"][L"error-text"].Get<Core::VTextLabel>()->SetPlaneText(L"The user name already exists!");
		}
	}
	void LoginToServer() {
		_bstr_t IPAddress  = this->operator[](L"main-widget")[L"login-interface"][L"login-block"][L"server-ip-editor"]
							 	.Get<Core::VLineEditor>()->GetPlaneText().c_str();
		...
		this->operator[](L"main-widget")[L"login-interface"][L"login-block"][L"error-text"].Get<Core::VTextLabel>()->SetPlaneText(L"");
		...
		if (connect(ClientSocket, (PSOCKADDR)&Address, sizeof(SOCKADDR_IN)) != 0) {
			this->operator[](L"main-widget")[L"login-interface"][L"login-block"][L"error-text"].Get<Core::VTextLabel>()->SetPlaneText(L"Failed to connect to server!");
	...
	void CheckFrame() override {
		...
		if (ResetMessageLabel) {
			...
			for (auto& Message : Messages) {
				...
				this->operator[](L"main-widget")[L"main-ui"][L"chat-editor"].Get<Core::VEditor>()->TextEffect.
					push_back({
							(ID2D1Effect*)UserBrush.GetDxBrush(), DWRITE_TEXT_RANGE{ (UINT32)PlaneText.size(), (UINT32)Message.UserNameString.size() + 1 }
						});
				...
				this->operator[](L"main-widget")[L"main-ui"][L"chat-editor"].Get<Core::VEditor>()->TextEffect.
					push_back({
							(ID2D1Effect*)TimeBrush.GetDxBrush(), DWRITE_TEXT_RANGE{ (UINT32)PlaneText.size(), (UINT32)lstrlen(TimeFormat) + 1 }
						});
				...
				this->operator[](L"main-widget")[L"main-ui"][L"chat-editor"].Get<Core::VEditor>()->TextEffect.
					push_back({
							(ID2D1Effect*)MessageBrush.GetDxBrush(), DWRITE_TEXT_RANGE{ (UINT32)PlaneText.size(), (UINT32)Message.MessageString.size() + 1 }
						});

				PlaneText = PlaneText + L"\n" + Message.MessageString;

				this->operator[](L"main-widget")[L"main-ui"][L"chat-editor"].Get<Core::VEditor>()->SetPlaneText(PlaneText);
...
public:
	ChatRoomClient(Core::VApplication* App) : VMLMainWindow(App), 
		UserBrush(Core::VColor::White, CallWidgetGetStaticRenderHandle()),
