...
void VGrayScaleEffect::ApplyEffect(const VRenderHandle &RenderHandle, Core::VCanvasPainter *Image)
{
...

	{
		...
		VLIB_CHECK_REPORT(FAILED(DeviceContext->CreateEffect(CLSID_D2D1Grayscale, &BlurEffect)),
						  L"Failed to create effect");
...
void VSepiaEffect::ApplyEffect(const VRenderHandle &RenderHandle, Core::VCanvasPainter *Image)
{
...

	{
		...
		VLIB_CHECK_REPORT(FAILED(DeviceContext->CreateEffect(CLSID_D2D1Sepia, &BlurEffect)),
						  L"Failed to create effect");

		BlurEffect->SetInput(0, Bitmap);
		BlurEffect->SetValue(D2D1_SEPIA_PROP_INTENSITY, Intensity);
		BlurEffect->GetOutput(&Output);
