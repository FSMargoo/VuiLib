...
bool VViewLabel::OnMessageTrigger(VRepaintMessage *RepaintMessage) {
    if (RepaintMessage->DirtyRectangle.Overlap(GetRegion()) &&
        ...
        if (ObjectVisual.Transparency != 0) {
            Canvas = new VCanvasPainter(ViewArea.GetWidth(),
                                        ViewArea.GetHeight(),
            ...
            VSolidBrush BackgroundBrush(CallWidgetGetDCRenderTarget()->GetDirectXRenderTarget(), Theme->BackgroundColor);
            VPenBrush BorderBrush(CallWidgetGetDCRenderTarget()->GetDirectXRenderTarget(), Theme->BorderColor);
