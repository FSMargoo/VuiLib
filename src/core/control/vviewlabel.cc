...
void VViewLabel::ResetScroller()
{
	...
	{
		VerticalScroller->Move(GetX() + GetWidth() - VerticalScroller->GetWidth(),
							   GetY() + HorizontalScroller->GetHeight());
	}
	...
	{
		HorizontalScroller->Move(GetX() + VerticalScroller->GetWidth(),
								 GetY() + GetHeight() - HorizontalScroller->GetHeight());
	}
...
void VViewLabel::EditChildMessage(VMessage *Message)
{
	...
	case VMessageType::MouseMoveMessage: {
		static_cast<VMouseMoveMessage *>(Message)->MousePosition.Offset(OffsetX, OffsetY);
...
bool VViewLabel::OnMessageTrigger(VRepaintMessage *RepaintMessage)
{
		...
		(GetParent()->IsApplication() || GetParent()->GetChildrenVisualRegion().Overlap(GetRegion())))
	{
		...
		if (ObjectVisual.Transparency != 0)
		{
			...
			Canvas->FillRoundedRectangle({OffsetX, OffsetY, OffsetX + GetWidth(), OffsetY + GetHeight()}, Theme->Radius,
			...
			if (ObjectVisual.Shadow.EnableStatus)
			{
				...
				GetParentCanvas()->DrawImage(
					{static_cast<int>(GetX() - ShadowOffset), static_cast<int>(GetY() - ShadowOffset),
					 static_cast<int>(GetX() - ShadowOffset + ShadowImage.GetWidth()),
					 static_cast<int>(GetY() - ShadowOffset + ShadowImage.GetHeight())},
...
VViewLabel::VViewLabel(const int &Width, const int &Height, const int &ViewWidth, const int &ViewHeight,
					   VUIObject *Parent)
	: VUIObject(Parent)
{
	Theme = new VViewLabelTheme(*(static_cast<VViewLabelTheme *>(GetTargetTheme(VUIThemeType::VViewLabel))));
