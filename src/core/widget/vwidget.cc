...
void VMainWindow::InitKernel() {
    FpsTimer.Start(14);
    _VMainConfigs.insert(std::pair<HWND, VMainWindowConfig*>(WindowHandle, &WindowConfig));
}
...
std::wstring VWidget::CallWidgetGetFocusID() {
    return WindowConfig.GlobalFocusID;
}
...
Core::VDCRender* VWidget::CallWidgetGetDCRenderTarget() {
    return Direct2DRender;
}
...
void VWidget::SetMaxSize(const VGeomtery& MaxSize) {
    WindowConfig.UseMaxMinSize = true;
    WindowConfig.WindowMaxSize = MaxSize;
}
void VWidget::SetMiniSize(const VGeomtery& MiniSize) {
    WindowConfig.UseMaxMinSize = true;
    WindowConfig.WindowMiniSize = MiniSize;
}
