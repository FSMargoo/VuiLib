...
void VMainWindow::InitWindow()
{
	...
	Direct2DRender =
		new VDCRender(VDirectXD2DFactory.GetInstance(), GetDC(WindowHandle), {0, 0, GetWidth(), GetHeight()}, true);
	BufferPainter =
		new VPainter(VRenderHandle(nullptr, nullptr, nullptr, Direct2DRender->GetDirectXRenderTarget(), nullptr));
	...
	if (FpsTimer.End())
	{
		...
		if (!RepaintMessages.empty())
		{
			...
			if (WindowConfig.EnableRadius)
			{
				...
				DCRenderTarget->FillRoundedRectangle(
					D2D1_ROUNDED_RECT{{0, 0, static_cast<float>(GetWidth() - 1), static_cast<float>(GetHeight() - 1)},
									  static_cast<float>(WindowConfig.BorderRadius.X),
									  static_cast<float>(WindowConfig.BorderRadius.Y)},
					BitmapBrush.GetDxBrush());
				...
				UpdateLayeredWindow(WindowHandle, WindowDC, NULL, &WindowSize, BorderImageDC, &SourcePoint, NULL,
									&BlendFN, ULW_ALPHA);
...
void VMainWindow::SetFrameless(const bool &FramelessStatus)
{
	if (FramelessStatus)
	{
		...
		SetWindowLongPtr(GetLocalWinId(), GWL_STYLE,
						 GetWindowLongPtr(GetLocalWinId(), GWL_STYLE) & ~(WS_THICKFRAME | WS_SIZEBOX | WS_CAPTION));
