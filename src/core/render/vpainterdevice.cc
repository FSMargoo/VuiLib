...
VDCRender::VDCRender(ID2D1Factory *DirectXFactory, const HDC &TargetDC, const VRect &DCRectangle, const bool &GDIMode,
					 const bool &HardwareAcceleration)
{
	...
	HRESULT ProduceResult = DirectXFactory->CreateDCRenderTarget(&Property, &DirectXDCTarget);
...
VHWNDRender::VHWNDRender(ID2D1Factory *DirectXFactory, const HWND &TargetHWND, const VRect &RenderRect,
						 const bool &HardwareAcceleration)
{
	VLIB_CHECK_REPORT(DirectXFactory == nullptr, L"Factory should not be nullptr!");
	...
	{
		Property =
			D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE::D2D1_RENDER_TARGET_TYPE_HARDWARE,
										 D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_STRAIGHT), 0.0,
										 0.0, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE, D2D1_FEATURE_LEVEL_DEFAULT);
	}
	else
	{
		Property =
			D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE::D2D1_RENDER_TARGET_TYPE_SOFTWARE,
										 D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_STRAIGHT), 0.0,
										 0.0, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE, D2D1_FEATURE_LEVEL_DEFAULT);
	}

	HRESULT ProduceResult = DirectXFactory->CreateHwndRenderTarget(
		D2D1::RenderTargetProperties(), D2D1::HwndRenderTargetProperties(TargetHWND, RenderRect.ToDxSize2U()),
		&DirectXDCTarget);
...
VIWICIRender::VIWICIRender(ID2D1Factory *DirectXFactory, IWICBitmap *TargetBitmap, const bool &HardwareAcceleration)
{
	...
	HRESULT ProduceResult =
		DirectXFactory->CreateWicBitmapRenderTarget((IWICBitmap *)TargetBitmap, (D2D1_RENDER_TARGET_PROPERTIES)Property,
													(ID2D1RenderTarget **)DirectXDCTarget.GetAddressOf());
