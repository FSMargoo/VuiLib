...
namespace Core {
    ...
    VImage::VImage(const int& Width, const int& Height, const VSurfaceAlphaMode& AlphaMode, const VRenderHandle& RenderHandle)
            : DirectXBitmap(RenderHandle.Allocator) {
        VLIB_REPORT_IF_FAILED_INFO(
        RenderHandle._IRenderTarget->CreateBitmap(
                D2D1::SizeU(Width, Height), 
    ...
    VImage::VImage(const std::wstring& FilePath, const VRenderHandle& RenderHandle)
            : DirectXBitmap(RenderHandle.Allocator) {
        ...
        VLIB_REPORT_IF_FAILED_INFO(
    	    IWICConverter->Initialize(IWICFrame, GUID_WICPixelFormat32bppPBGRA,
    ...
    void VImage::ApplyGassBlur(const int &Radius, const VRenderHandle& RenderHandle) {
        ...
        VLIB_CHECK_REPORT(FAILED(DeviceContext->CreateEffect(CLSID_D2D1GaussianBlur, &BlurEffect)), L"Failed to create effect");
        ...
        RenderHandle._IRenderTarget->CreateCompatibleRenderTarget(D2D1_SIZE_F { static_cast<float>(OutputRectangle.right + abs(OutputRectangle.left)),
        ...
        ResultDeviceContext->DrawImage(BlurOutput.Get(), D2D1_POINT_2F{abs(OutputRectangle.left), abs(OutputRectangle.top)});
    
        ResultDeviceContext->EndDraw();

        D2D1_POINT_2U OriginPoint = { 0, 0 };
        D2D1_RECT_U   Rect        = { 0, 0,
                                      static_cast<unsigned int>(OutputRectangle.right  + abs(OutputRectangle.left)),
    ...
    void VImage::ApplyShadowEffect(const float &ShadowRadius, const VColor &ShadowColor, const VRenderHandle& RenderHandle, VPoint* Offset) {
        ...
        ResultDeviceContext->Clear(D2D1::ColorF(0.f, 0.f, 0.f, 0.f));
        ResultDeviceContext->DrawImage(EffectOutput.Get(), D2D1_POINT_2F{abs(OutputRectangle.left), abs(OutputRectangle.top)});
    ...
    VColor VImage::GetPixel(const int& X, const int& Y, const VRenderHandle& RenderHandle) const {
        ...
        VLIB_REPORT_IF_FAILED_INFO(
            RenderHandle._IRenderTarget->QueryInterface(&DeviceContext),
            L"Failed to query interface from render target"
        );
