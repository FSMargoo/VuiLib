 ...
 * Date			: 11/9/2022
 * Description
 * : The most basic definition of objects and functions in vuilib
 */
#pragma once
 ...
 *   Description : If user define the VLIB_ENABLE_NAMESAPCE
 * Marco, all the
 *VuiLib functional will be include in a namespace called VuiLib
 */
 ...
 *  Description : Check the Win32 operation (or any operation that return
 *HRESULT
 * as status code), if the operation failed   (that is, ReturnValue !=
 *S_OK), it will report operation failed.
 */
 ...
 * *HRESULT as status code), if the operation failed   (that is, ReturnValue !=
 *S_OK), it will report operation
 * failed. Tips		: Not same as
 *VLIB_RERPOT_IF_FAILED it can bring message to debug widget
 */
#define VLIB_REPORT_IF_FAILED_INFO(Expression, WidgetInformation)                                                      \
	{                                                                                                                  \
		if ((Expression) != S_OK)                                                                                      \
		{                                                                                                              \
			VLIB_CHECK_REPORT(true, WidgetInformation);                                                                \
		}                                                                                                              \
	}
...
public:
   ...
   *
	 * Tips		: For example, if this rectangle's area is [ 40,
   *50, 80, 10 ], a rectangle provided's area
	 * is [ 30, 60, 100, 40 ], after
   *FusionRect, this rectangle's value will change into [ 30, 50, 100, 40 ],

	 * *therefore, the new rectangle's area will be: [ min(R1'left, R2'left),
   *min(R1'top, R2'top), max(R1'right,
	 * R2'right), max(R1'bottom, R2'bottom)
   *]
   */
	...
	/*
   * Overlap function:
   *		Description : Judge a target rectangle overlap or not with this

	 * *rectangle, if it overlaps with this rectangle, return true, or false Tips
   *: Not same as Include, Overlap
	 * will return true whenever there is an
   *intersection area exists
   */
...
public:
   ...
   *		Description : Judge the point inside the target rectangle or

	 * *not, if the point inside the target rectangle, return true, nor false
   */
...
public:
	...
	/*
   * ToDxPointU:
   *		Description : Convert the VPoint into DirectX's Point2U
   *structure
	...
	/*
   * ToDxPOINT2F:
   *		Description : Convert the VPoint into DirectX's Point2F
   *structure
 ...
 *	Description : This class describe a rectangle, but not same as VRect, it
 ...
 *	Description : The rectangle structure which only support double
 * data
 *type
 */
 ...
 *	Description : The point structure which only support int data
 * type
 */
using VPoint = VCorePoint<int>;
/*
 * VPointF (alias from VCorePoint<double>):
 *	Description : The point structure which only support double data
 * type
 */
using VPointF = VCorePoint<double>;
/*
 * VGeometry (alias from VCoreGeometry<int>):
 *	Description : The geometry structure which only support int data
 * type
 */
