...
sk_sp<Texture> ResourceProvider::findOrCreateDepthStencilAttachment(SkISize dimensions,
    ...
    GraphiteResourceKey key;
    // We always make depth and stencil attachments shareable. Between any renderer pass the values
    // are reset. Thus it is safe to be used by multiple different renderer passes without worry of
...
sk_sp<Texture> ResourceProvider::findOrCreateDiscardableMSAAAttachment(SkISize dimensions,
    ...
    GraphiteResourceKey key;
    // We always make discardable msaa attachments shareable. Between any renderer pass we discard
    // the values of the MSAA texture. Thus it is safe to be used by multiple different renderer
    // passes without worry of stomping on each other's data. It is the callings code responsiblity
    // to populate the discardable MSAA texture with data at the start of the renderer pass.
    fGpu->caps()->buildKeyForTexture(dimensions, info, kType, Shareable::kYes, &key);
