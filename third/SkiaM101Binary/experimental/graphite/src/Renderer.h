...
private:
    ...
    // Alternatively, if we imposed a max attr count, similar to Renderer's num renderer steps, we
...
public:
    ...
    // The maximum number of renderer steps that any Renderer is allowed to have.
...
private:
    // max renderer steps is 4, so just spell the options out for now...
