...
#undef ApplyLazyInheritedAttribute
    ...
    // TODO: resolve these in a pre-renderer styling pass and assert here that they are values.
    // - stop-color
    // - stop-opacity
    // - flood-color
    // - flood-opacity
...
void SkSVGRenderContext::applyMask(const SkSVGFuncIRI& mask) {
    ...
    // The pending layers are restored in the destructor (renderer context scope exit).
...
SkTLazy<SkPaint> SkSVGRenderContext::commonPaint(const SkSVGPaint& paint_selector,
    ...
    case SkSVGPaint::Type::kIRI: {
        // Our property inheritance is borked as it follows the renderer path and not the tree
        ...
        // (e.g. gradient control points), which requires access to the renderer context
