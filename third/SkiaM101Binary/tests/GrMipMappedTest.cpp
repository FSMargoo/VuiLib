...
// Create a new renderer target and draw 'mipmapView' into it using the provided 'filter'.
...
DEF_GPUTEST(GrManyDependentsMipMappedTest, reporter, /* options */) {
    ...
    for (auto enableSortingAndReduction : {Enable::kYes, Enable::kNo}) {
        ...
        // Create a mipmapped renderer target.
        ...
        // mipmapProxy's last renderer task should now just be the opsTask containing the clear.
        REPORTER_ASSERT(reporter,
        ...
        // Draw the dirty mipmap texture into a renderer target.
        ...
        // Draw the dirty mipmap texture into a renderer target, but don't do mipmap filtering.
        ...
        // Since mips weren't regenerated, the last renderer task shouldn't have changed.
        REPORTER_ASSERT(reporter,
                    mipmapRTCTask.get() == drawingManager->getLastRenderTask(mipmapProxy.get()));
        ...
        // Make sure the mipmap texture now has a new last renderer task that regenerates the mips,
