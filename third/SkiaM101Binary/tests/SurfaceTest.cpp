    ...
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
        ...
        for (int sampleCnt : {1, 2}) {
            ...
            if (!surf && can && colorType == kBGRA_8888_SkColorType && sampleCnt > 1 &&
                ...
                // created renderer targets by using a MSAA RGBA8 renderbuffer that resolves to a
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            if (pixel != expectedValue) {
                SkString msg;
                if (expectedValue) {
                    msg = "SkSurface should have left renderer target unmodified";
                } else {
                    msg = "SkSurface should have cleared the renderer target";
                }
                ERRORF(reporter,
