...
public:
    ...
    */
    ...
    /** User function called when supplied renderer target may be deleted. */
    ...
    */
    ...
    /** Wraps a GPU-backed buffer into SkSurface. Caller must ensure backendRenderTarget
        ...
        not exceed context capabilities, and the context must be able to support
        back-end renderer targets.

        Upon success releaseProc is called when it is safe to delete the renderer target in the
        backend API (accounting only for use of the renderer target by this surface). If SkSurface
    ...
    */
    ...
    /** Retrieves the back-end renderer target. If SkSurface has no back-end renderer target, an invalid
        ...
        @return                     GPU renderer target reference; invalid on failure
    */
    ...
    */
    ...
    /** If a surface is GPU texture backed, is being drawn with MSAA, and there is a resolve
        ...
        texture, but asking Skia to renderer with MSAA. If the client wants to use the wrapped texture
