...
enum class GrInternalSurfaceFlags {
    ...
    // This flag is for use with GL only. It tells us that the internal renderer target wraps FBO 0.
    kGLRTFBOIDIs0                   = 1 << 1,

    // This means the renderer target is multisampled, and internally holds a non-msaa texture for
    // resolving into. The renderer target resolves itself by blitting into this internal texture.
    ...
    // This means the pixels in the renderer target are write-only. This is used for Dawn and Metal
