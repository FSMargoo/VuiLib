...
public:
    ...
    enum class ResolveFlags {
        kNone = 0,
        kMSAA = 1 << 0,  // Blit and resolve an internal MSAA renderer buffer into the texture.
    ...
    /**
     * @return the renderer target proxy associated with the surface proxy, may be NULL.
     */
    ...
    /** How many renderer tasks has this proxy been the target of? */
    ...
    // If this is a renderer target proxy and the proxy is already instantiated, return its backing
    ...
    /**
     * This means surface is a multisampled renderer target, and internally holds a non-msaa texture
     * for resolving into. The renderer target resolves itself by blitting into this internal texture.
     * (asTexture() might or might not return the internal texture, but if it does, we always
     * resolve the renderer target before accessing this texture's data.)
     */
    ...
    // The copy is is not a renderer target and not multisampled.
    //
    ...
    //
    // Optionally gets the renderer task that performs the copy. If it is later determined that the
