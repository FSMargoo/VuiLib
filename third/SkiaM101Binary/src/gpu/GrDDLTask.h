...
/**
 * This renderer task isolates the DDL's tasks from the rest of the DAG. This means that
 * the DDL's tasks cannot be reordered by the topological sort and are always executed
 * as a single block.
 * It almost entirely just forwards calls down to the DDL's renderer tasks.
 */
...
public:
    ...
    // The renderer tasks w/in the DDL don't appear in the DAG so need explicit notification
