...
bool SurfaceContext::internalWritePixels(GrDirectContext* dContext,
    ...
    if ((!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) && numLevels == 1) {
        ...
        // However, if the final proxy isn't a renderer target then we must use a copy to move the
        // data into it which requires the origins to match. If the final proxy is a renderer target
        // we can use a draw instead which doesn't have this origin restriction. Thus for renderer
