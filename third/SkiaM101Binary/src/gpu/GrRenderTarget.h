 ...
 * A context's renderer target is set by setRenderTarget(). Render targets are
 * created by a createTexture with the kRenderTarget_SurfaceFlag flag.
 * Additionally, GrContext provides methods for creating GrRenderTargets
 * that wrap externally created renderer targets.
 */
...
public:
    ...
    /**
     * Returns a unique key that identifies this renderer target's sample pattern. (Must be
     * multisampled.)
     */
    int getSamplePatternKey();

    /**
     * Retrieves the per-pixel HW sample locations for this renderer target, and, as a by-product, the
