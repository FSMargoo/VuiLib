...
// before we renderer the next one in line. This means there will only ever be one atlas active at a
...
bool AtlasPathRenderer::addPathToAtlas(GrRecordingContext* rContext,
    ...
    // bounds within the max renderer target size are nowhere near INT_MAX.
    auto [topLeftFloor, botRightCeil] = round_out(pathDevBounds);
