...
/**
 * The scissor state is stored as the scissor rectangle and the backing store bounds of the renderer
 * target that the scissor will apply to. If the renderer target is approximate fit and the padded
 * content should not be modified, the clip should apply the renderer target context's logical bounds
 * as part of the scissor (e.g. when stenciling). This puts the onus on the renderer target context
...
public:
    // The disabled scissor state for a renderer target of the given size.
    ...
    bool enabled() const {
        ...
        // the renderer target bounding rectangle.
    ...
    // added that did not intersect with the renderer target or prior scissor.
...
private:
    // The scissor is considered enabled if the rectangle does not cover the renderer target
    SkISize fRTSize;
    SkIRect fRect;
};
