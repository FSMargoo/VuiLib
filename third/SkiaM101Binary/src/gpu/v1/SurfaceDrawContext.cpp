...
void SurfaceDrawContext::drawGlyphRunList(SkCanvas* canvas,
    ...
    // secondary command buffers because it would require stopping and starting a renderer pass which
...
void SurfaceDrawContext::drawPaint(const GrClip* clip,
    ...
    // Start with the renderer target, since that is the maximum content we could possibly fill.
...
enum class SurfaceDrawContext::QuadOptimization {
    // The rect to draw doesn't intersect clip or renderer target, so no draw op should be added
    kDiscarded,
    ...
    kClipApplied,
    // No change to clip, but quad updated to better fit clip/renderer target, and caller is
...
SurfaceDrawContext::QuadOptimization SurfaceDrawContext::attemptQuadOptimization(
    ...
    // Use the logical size of the renderer target, which allows for "fullscreen" clears even if
    // the renderer target has an approximate backing fit
    ...
    auto conservativeCrop = [&]() {
        ...
        // Crop the quad to the renderer target. This doesn't change the visual results of drawing but
    ...
    switch(result.fEffect) {
        ...
        case GrClip::Effect::kUnclipped:
            if (!simpleColor) {
                ...
                // Update result to store the renderer target bounds in order and then fall
...
OpsTask::CanDiscardPreviousOps SurfaceDrawContext::canDiscardPreviousOpsOnFullClear() const {
    ...
    // would normally be overwritten. The one exception is if the renderer target context is marked as
...
bool SurfaceDrawContext::drawFastShadow(const GrClip* clip,
    ...
    if (SkColorGetA(rec.fSpotColor) > 0) {
        ...
        if (viewMatrix.invert(&ctmInverse)) {
            ...
            SkDebugf("Matrix is degenerate. Will not renderer spot shadow correctly!\n");
            SkASSERT(false);
        }
...
void SurfaceDrawContext::addDrawOp(const GrClip* clip,
    ...
    if (clip) {
        ...
        // No clipping, so just clip the bounds against the logical renderer target dimensions
    ...
    // preserved between renderer passes, if needed.
    ...
    if (analysis.requiresDstTexture()) {
        ...
        if (fCanUseDynamicMSAA && drawNeedsMSAA && !this->caps()->msaaResolvesAutomatically()) {
            // Since we aren't literally writing to the renderer target texture while using a DMSAA
...
bool SurfaceDrawContext::setupDstProxyView(const SkRect& opBounds,
    ...
    // start and stop the renderer pass in order to make the copy.
    ...
    // msaaResolvesAutomatically (i.e. renderer-to-msaa-texture). In that case there will be no op or
    // barrier between draws to flush the renderer target before being used as a texture in the next
