...
public:
     ...
     * uses separate MSAA renderer buffers then a MSAA renderer buffer is created that resolves
     * to the texture.
     */
    ...
    /**
     * Wraps an existing renderer target with a GrRenderTarget object. It is
     * similar to wrapBackendTexture but can be used to draw into surfaces
     * that are not also textures (e.g. FBO 0 in OpenGL, or an MSAA buffer that
     * the client will resolve to a texture). Currently wrapped renderer targets
    ...
    /**
     * Returns an index buffer that can be used to renderer non-antialiased quads.
    ...
    /**
     * Returns an index buffer that can be used to renderer antialiased quads.
    ...
    /**
     * If passed in renderer target already has a stencil buffer on the specified surface, return
     ...
     * data at the start and should not try to save written data at the end. Ideally the renderer pass
