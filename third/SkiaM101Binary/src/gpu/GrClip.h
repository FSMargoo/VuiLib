...
public:
    ...
    /**
     * Compute a conservative pixel bounds restricted to the given renderer target dimensions.
     ...
     * clip.
     *
     * When set, 'result.fRRect' will intersect with the renderer target bounds but may extend
     * beyond it. If the renderer target bounds are the only clip effect on the draw, this is reported
