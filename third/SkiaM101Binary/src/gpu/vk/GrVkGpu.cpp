...
GrOpsRenderPass* GrVkGpu::onGetOpsRenderPass(
    ...
    // For the given renderer target and requested renderer pass features we need to find a compatible
    // framebuffer to use for the renderer pass. Technically it is the underlying VkRenderPass that
    ...
    // Covert the GrXferBarrierFlags into renderer pass self dependency flags
    ...
    // Figure out if we need a resolve attachment for this renderer pass. A resolve attachment is
    ...
    // Get the framebuffer to use for the renderer pass
...
bool GrVkGpu::onTransferPixelsFrom(GrSurface* surface,
    ...
    if (GrVkRenderTarget* rt = static_cast<GrVkRenderTarget*>(surface->asRenderTarget())) {
        // Reading from renderer targets that wrap a secondary command buffer is not allowed since
...
void GrVkGpu::onResolveRenderTarget(GrRenderTarget* target, const SkIRect& resolveRect) {
    ...
    if (this->vkCaps().renderTargetSupportsDiscardableMSAA(rt)) {
        // We would have resolved the RT during the renderer pass;
        return;
    }
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT) {
    ...
    // We will always renderer directly to this VkImage.
...
bool GrVkGpu::createVkImageForBackendSurface(VkFormat vkFormat,
    ...
    if (renderable == GrRenderable::kYes) {
        ...
        // We always make our renderer targets support being used as input attachments
...
void GrVkGpu::endRenderPass(GrRenderTarget* target, GrSurfaceOrigin origin,
                            const SkIRect& bounds) {
    // We had a command buffer when we started the renderer pass, we should have one now as well.
    SkASSERT(this->currentCommandBuffer());
