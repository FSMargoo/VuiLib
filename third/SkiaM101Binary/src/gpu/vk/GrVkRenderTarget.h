...
public:
    ...
    // Reconstruct the renderer target attachment information from the programInfo. This includes
    // which attachments the renderer target will have (color, stencil) and the attachments' formats
...
private:
    ...
    // these being completely orthogonal. Thus we have a total of 32 types of renderer passes. We then
    // cache a framebuffer for each type of these renderer passes.
