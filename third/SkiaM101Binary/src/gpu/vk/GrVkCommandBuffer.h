...
public:
    ...
    // Commands that only work inside of a renderer pass
...
protected:
    ...
    // Stores a pointer to the current active renderer pass (i.e. begin has been called but not
    // end). A nullptr means there is no active renderer pass. The GrVKCommandBuffer does not own
    // the renderer pass.
...
public:
    ...
    // Begins renderer pass on this command buffer. The framebuffer from GrVkRenderTarget will be used
    ...
    // currently inside a renderer pass that is compatible with the one used to create the
    // SecondaryCommandBuffer.
    void executeCommands(const GrVkGpu* gpu,
                         std::unique_ptr<GrVkSecondaryCommandBuffer> secondaryBuffer);

    // Commands that only work outside of a renderer pass
