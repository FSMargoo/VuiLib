...
void GrVkCommandBuffer::pipelineBarrier(const GrVkGpu* gpu,
    ...
    SkASSERT(fIsActive);
#ifdef SK_DEBUG
    // For images we can have barriers inside of renderer passes but they require us to add more
    // support in subpasses which need self dependencies to have barriers inside them. Also, we can
    // never have buffer barriers inside of a renderer pass. For now we will just assert that we are
    // not in a renderer pass.
...
void GrVkCommandBuffer::submitPipelineBarriers(const GrVkGpu* gpu, bool forSelfDependency) {
    ...
    if (fBufferBarriers.count() || fImageBarriers.count()) {
        ...
        // can never have buffer barriers inside of a renderer pass. For now we will just assert that
        // we are not in a renderer pass.
...
GrVkPrimaryCommandBuffer::~GrVkPrimaryCommandBuffer() {
    // Should have ended any renderer pass we're in the middle of
    SkASSERT(!fActiveRenderPass);
}
...
void GrVkPrimaryCommandBuffer::executeCommands(const GrVkGpu* gpu,
    ...
    // When executing a secondary command buffer all state (besides renderer pass state) becomes
...
void GrVkSecondaryCommandBuffer::begin(GrVkGpu* gpu, const GrVkFramebuffer* framebuffer,
    ...
    inheritanceInfo.subpass = 0; // Currently only using 1 subpass for each renderer pass
