...
bool GrVkCaps::onCanCopySurface(const GrSurfaceProxy* dst, const GrSurfaceProxy* src,
    ...
    if (const GrRenderTargetProxy* rtProxy = dst->asRenderTargetProxy()) {
        // Copying to or from renderer targets that wrap a secondary command buffer is not allowed
...
void GrVkCaps::applyDriverCorrectnessWorkarounds(const VkPhysicalDeviceProperties& properties) {
    ...
    // On Qualcomm and Arm the gpu resolves an area larger than the renderer pass bounds when using
 ...
 * pipline. This includes stencil settings, blending information, renderer pass format, draw face
