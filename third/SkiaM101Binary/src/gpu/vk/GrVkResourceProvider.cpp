...
// only used for framebuffer creation. When we actually renderer we will create
...
GrVkResourceProvider::findCompatibleRenderPass(GrVkRenderTarget* target,
    ...
    // Get attachment information from renderer target. This includes which attachments the renderer
...
void GrVkResourceProvider::destroyResources() {
    ...
    // loop over all renderer pass sets to make sure we destroy all the internal VkRenderPasses
    for (int i = 0; i < fRenderPassArray.count(); ++i) {
