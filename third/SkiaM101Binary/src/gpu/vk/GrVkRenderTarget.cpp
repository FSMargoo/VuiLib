...
GrVkRenderTarget::GrVkRenderTarget(GrVkGpu* gpu,
    ...
    if (fColorAttachment->numSamples() == 1 && fColorAttachment->supportsInputAttachmentUsage()) {
        ...
        // to the same attachment. This way if we use DMAA on this renderer target we will resolve
...
GrBackendRenderTarget GrVkRenderTarget::getBackendRenderTarget() const {
    ...
    // If we have a resolve attachment that is what we return for the backend renderer target
