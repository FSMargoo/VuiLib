...
public:
    ...
    // Used when importing an external renderer pass. In this case we have to explicitly be told the
    ...
    enum AttachmentFlags : uint32_t {
        ...
        // The external attachment flag signals that this renderer pass is imported from an external
        // client. Since we don't know every attachment on the renderer pass we don't set any of the
        // specific attachment flags when using external. However, the external renderer pass must
    ...
    // The following return the index of the renderer pass attachment array for the given attachment.
    // If the renderer pass does not have the given attachment it will return false and not set the
    // index value.
    ...
    // Returns the number of clear colors needed to begin this renderer pass. Currently this will
...
private:
    ...
    // For internally created renderer passes we assume the color attachment index is always 0.
