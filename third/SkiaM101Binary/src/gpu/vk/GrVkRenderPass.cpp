...
GrVkRenderPass* GrVkRenderPass::Create(GrVkGpu* gpu,
    ...
    // sense not to store the resolve attachment at the end of the renderer pass.
    //
    ...
    // the resolving at the end of the last renderer pass, we would probably want a separate flag
    // for having a resolve attachment versus actually doing the resolving. This would allow us to
    // use the same VkPiplines for renderer passes where we resolve and those we don't since each will
    // always have the resolve attachment. The actual resolving or not does not affect renderer pass
    ...
    // Attachment descriptions to be set on the renderer pass
    ...
    // Refs to attachments on the renderer pass (as described by the VkAttachmentDescription above),
    ...
    // Get granularity for this renderer pass
