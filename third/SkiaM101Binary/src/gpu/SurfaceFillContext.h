...
public:
    ...
    /**
     * Provides a performance hint that the renderer target's contents are allowed
     * to become undefined.
     */
    ...
    /**
     * Clear the rect of the renderer target to the given color.
    ...
    /** Clears the entire renderer target to the color. */
    ...
    /**
     * Clear at minimum the pixels within 'scissor', but is allowed to clear the full renderer target
    ...
    /** Fills the entire renderer target with the passed FP. */
     ...
     * GrMatrixEffect and fills the entire renderer target.
     */
