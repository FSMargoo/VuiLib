...
public:
     ...
     * be used as a renderer target by calling GrTexture::asRenderTarget(). Not all
     * pixel configs can be used as renderer targets. Support for configs as textures
     * or renderer targets can be checked using GrCaps.
     *
    ...
    /**
     * Reads a rectangle of pixels from a renderer target. No sRGB/linear conversions are performed.
     *
    ...
    // renderer target's stencil buffer should be ignored.
    ...
    /**
     * In this case we have a program descriptor and a program info but no renderer target.
     */
