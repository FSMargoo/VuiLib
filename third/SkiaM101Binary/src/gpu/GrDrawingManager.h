...
public:
    ...
    // Create a renderer task that can resolve MSAA and/or regenerate mipmap levels on proxies. This
    // method will only add the new renderer task to the list. However, it adds the task before the
    ...
    // Creates a renderer task that can resolve MSAA and/or regenerate mimap levels on the passed in
    ...
    // Create a new renderer task that will cause the gpu to wait on semaphores before executing any
    ...
    // Create a new renderer task which copies the pixels from the srcProxy into the dstBuffer. This
    ...
    // Creates a new renderer task which copies a pixel rectangle from srcView into dstView. The src
...
private:
    ...
    // Attempt to reorder tasks to reduce renderer passes, and check the memory budget of the
