...
public:
    ...
    // an MSAA-renderer-to-texture extension: Any renderer target we create internally will use the
    // extension, and any wrapped renderer target is the client's responsibility.
    ...
    // and stores. The use of this feature for specific draws depends on the renderer target having a
    ...
    // This flag is similar to enabling gl renderer to texture for msaa rendering.
    ...
    // Should we discard stencil values after a renderer pass? (Tilers get better performance if we
    ...
    // that we upload to (i.e., don't have a renderer target) will not be
    ...
    /** This is the largest renderer target size that can be used without incurring extra perfomance
        cost. It is usually the max RT size, unless larger renderer targets are known to be slower. */
    ...
    // Returns whether window rectangles are supported for the given backend renderer target.
    ...
    // renderer target of the given format or 0 if no such sample count is supported. If the requested
    ...
    enum class ProgramDescOverrideFlags {
        kNone = 0,
        // If using discardable msaa surfaces in vulkan, when we break up a renderer pass for an
        // inline upload, we must do a load msaa subpass for the second renderer pass. However, if the
        // original renderer pass did not have this load subpass (e.g. clear or discard load op), then
        // all the GrProgramInfos for draws that end up in the second renderer pass will have been
        // recorded thinking they will be in a renderer pass with only 1 subpass. Thus we add an
        // override flag to the makeDesc call to force the actually VkPipeline that gets created to
        // be created using a renderer pass with 2 subpasses. We do miss on the pre-compile with this
