...
GrRenderTargetProxy::GrRenderTargetProxy(sk_sp<GrSurface> surf,
    ...
    // texture as a renderer target at this point but we wouldn't know it.
...
bool GrRenderTargetProxy::canUseStencil(const GrCaps& caps) const {
    ...
    if (!this->isInstantiated()) {
        if (this->isLazy() && this->backendFormat().backend() == GrBackendApi::kOpenGL) {
            // It's possible for wrapped GL renderer targets to not have stencil. We don't currently
            ...
            // stencils on internal renderer targets.
            return true;
        }
    }
