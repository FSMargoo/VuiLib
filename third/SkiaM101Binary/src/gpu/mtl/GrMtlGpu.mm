...
GrOpsRenderPass* GrMtlGpu::onGetOpsRenderPass(
    ...
    // Figure out if we can use a Resolve store action for this renderer pass. When we set up
    // the renderer pass we'll update the color load/store ops since we don't want to ever load
...
bool GrMtlGpu::readOrTransferPixels(GrSurface* surface,
    ...
    if (GrMtlRenderTarget* rt = static_cast<GrMtlRenderTarget*>(surface->asRenderTarget())) {
        if (rt->numSamples() > 1) {
            SkASSERT(rt->requiresManualMSAAResolve());  // msaa-renderer-to-texture not yet supported.
