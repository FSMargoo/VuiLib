...
public:
    ...
    // Binds the renderer target to GL_FRAMEBUFFER for rendering.
    void bind(bool useMultisampleFBO) {
        this->bindInternal(GR_GL_FRAMEBUFFER, useMultisampleFBO);
    }

    // Must be rebound even if this is already the currently bound renderer target.
    bool mustRebind(bool useMultisampleFBO) const {
        return fNeedsStencilAttachmentBind[useMultisampleFBO];
    }

    // Binds the renderer target for copying, reading, or clearing pixel values. If we are an MSAA
    // renderer target with a separate resolve texture, we bind the multisampled FBO. Otherwise we
...
protected:
    ...
    // Binds the renderer target to the given target and ensures its stencil attachment is valid.
