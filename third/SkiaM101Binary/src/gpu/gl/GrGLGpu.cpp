...
void GrGLGpu::onResetContext(uint32_t resetBits) {
    if (resetBits & kMisc_GrGLBackendState) {
        ...
        if (GR_IS_GR_GL_ES(this->glStandard()) &&
            ...
            // On some devices this may have a perf hit.  Also multiple renderer targets are disabled
...
bool GrGLGpu::createRenderTargetObjects(const GrGLTexture::Desc& desc,
    ...
    // If we are using multisampling we will create two FBOS. We renderer to one and then resolve to
...
void GrGLGpu::unbindSurfaceFBOForPixelOps(GrSurface* surface, int mipLevel, GrGLenum fboTarget) {
    // bindSurfaceFBOForPixelOps temporarily binds textures that are not renderer targets to
    ...
    switch (type) {
        case kTexture_GrXferBarrierType: {
            ...
            if (glrt->requiresManualMSAAResolve()) {
                // The renderer target uses separate storage so no need for glTextureBarrier.
                // FIXME: The renderer target will resolve automatically when its texture is bound,
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions,
    ...
    // We make a texture instead of a renderer target if we're using a
    // "multisampled_render_to_texture" style extension or have a BGRA format that
    // is allowed for textures but not renderer buffer internal formats.
    ...
    if (sampleCnt > 1 && !this->glCaps().usesMSAARenderBuffers()) {
        ...
        // We have a BGRA extension that doesn't support BGRA renderer buffers. We can use a texture
        // unless we've been asked for MSAA. Note we already checked above for renderer-to-
