...
private:
    ...
    // Helper for setData() that sets the view matrix and loads the renderer target height uniform
    void setRenderTargetState(const GrRenderTarget*, GrSurfaceOrigin);

    sk_sp<GrD3DPipeline> fPipeline;
    sk_sp<GrD3DRootSignature> fRootSignature;

    // Tracks the current renderer target uniforms stored in the vertex buffer.
