...
int GrMockGpu::NextExternalRenderTargetID() {
    // We use large negative ints for the "testing only external renderer targets" so they can easily
...
sk_sp<GrTexture> GrMockGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex,
    ...
    // The client gave us the texture ID but we supply the renderer target ID.
