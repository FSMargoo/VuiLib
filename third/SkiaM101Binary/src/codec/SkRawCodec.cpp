...
public:
    ...
    dng_image* render(int width, int height) {
        ...
        try {
            // renderer() takes ownership of fHost, fInfo, fNegative and fDngStream when available.
...
SkCodec::Result SkRawCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst,
    ...
    // Because the DNG SDK can not guarantee to renderer to requested size, we allow a small
