...
static GrRenderTarget* prepare_rt_for_external_access(SkSurface_Gpu* surface,
    ...
    // Grab the renderer target *after* firing notifications, as it may get switched if CoW kicks in.
...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
    ...
    if (subset || !srcView.asTextureProxy() || rtp->refsWrappedObjects()) {
        // If the original renderer target is a buffer originally created by the client, then we don't
...
// Create a new renderer target and, if necessary, copy the contents of the old
// renderer target into it. Note that this flushes the SkGpuDevice but
...
bool SkSurface_Gpu::onCharacterize(SkSurfaceCharacterization* characterization) const {
    ...
    // We should never get in the situation where we have a texture renderer target that is also
    // backend by FBO 0.
