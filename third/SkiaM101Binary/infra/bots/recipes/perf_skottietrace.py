...
RENDER_TRACE_NAME = 'skottie::Animation::renderer'
...
def parse_trace(trace_json, lottie_filename, api):
  ...
  considered the same as seek time + renderer time.
    ...
    if '%s' in trace['name']:
      if frame_start:
        raise Exception('We got consecutive Animation::seek without a ' +
                        'renderer. Something is wrong.')
      frame_start = True
      current_frame_duration = trace['dur']
    elif '%s' in trace['name']:
      if not frame_start:
        raise Exception('We got an Animation::renderer without a seek first. ' +
                        'Something is wrong.')
