 ...
 * renderer target and syncs the GPU after each draw.
 *
...
static DEFINE_bool(comparableDDL, false, "renderer in a way that is comparable to 'comparableSKP'");
...
int main(int argc, char** argv) {
    ...
    if (ctx->maxRenderTargetSize() < std::max(width, height)) {
        exitf(ExitErr::kUnavailable, "renderer target size %ix%i not supported by platform (max: %i)",
    ...
    if (!surface) {
        exitf(ExitErr::kUnavailable, "failed to create %ix%i renderer target for config %s",
