...
void DebugCanvas::drawTo(SkCanvas* originalCanvas, int index, int m) {
    ...
    if (at) {
        ...
        // get the renderer target of the top device (from the original canvas) so we can ignore ops
        // drawn offscreen
...
void DebugCanvas::onDrawAnnotation(const SkRect& rect, const char key[], SkData* value) {
    ...
    if (tokens.size() == 2) {
        if (tokens[0].equals(kOffscreenLayerDraw)) {
            // Indicates that the next drawPicture command contains the SkPicture to renderer the
...
void DebugCanvas::onDrawImageRect2(const SkImage*           image,
    ...
    if (fnextDrawImageRectLayerId != -1 && fLayerManager) {
        ...
        // To renderer the layer as it would have looked on the frame this DebugCanvas draws, we need
